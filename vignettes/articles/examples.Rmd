...
## `rand_forest()` models
  ...
  lung_train_orsf <- lung_train
  lung_train_orsf$status <- lung_train_orsf$status - 1
  
  rf_fit <- rf_spec %>% fit(Surv(time, status) ~ ., data = lung_train_orsf)
  rf_fit
  ```
  ...
  predict(
    ...
    lung_test, 
    type = "survival", 
    # aorsf is picky with inputs, it does not let you
    # set a prediction horizon > max(observed follow-up time)
    time = c(100, 500, max(lung_train_orsf$time))
  ) %>% 
    slice(1) %>% 
    tidyr::unnest(col = .pred)
  ```
