...
## `bag_tree()` models
  ...
  ```{r}
  predict(bt_fit, lung_test, type = "survival", time = c(100, 500, 1000)) %>% 
    slice(1) %>% 
...
## `decision_tree()` models
  ...
  ```{r}
  predict(dt_fit, lung_test, type = "survival", time = c(100, 500, 1000)) %>% 
    slice(1) %>% 
    tidyr::unnest(col = .pred)
  ...
  <details id="decision-tree-party"> 
  
  <summary>With the `"party"` engine</summary>
...
## `proportional_hazards()` models
  ...
  ```{r}
  predict(ph_fit, lung_test, type = "survival", time = c(100, 500, 1000)) %>% 
    slice(1) %>% 
    tidyr::unnest(col = .pred)
...
## `rand_forest()` models
  ...
  ```{r}
  predict(rf_fit, lung_test, type = "survival", time = c(100, 500, 1000)) %>% 
    slice(1) %>% 
    tidyr::unnest(col = .pred)
...
## `survival_reg()` models
  ...
  The holdout data can be predicted for survival probability at different time points as well as event time, quantile, and hazard.
  
  ```{r}
  predict(sr_fit, lung_test, type = "survival", time = c(100, 500, 1000)) %>% 
    slice(1) %>% 
    tidyr::unnest(col = .pred)
  ...
  predict(sr_fit, lung_test, type = "quantile") %>% 
    slice(1) %>% 
    tidyr::unnest(col = .pred)
  predict(sr_fit, lung_test, type = "hazard", time = c(100, 500, 1000)) %>% 
    slice(1) %>% 
    tidyr::unnest(col = .pred)
  ```
  ...
  ```{r}
  set.seed(1)
  sr_fit <- sr_spec %>% fit(Surv(time, status) ~ age + sex + ph.ecog, data = lung_train)
  sr_fit
  ```

  The holdout data can be predicted for survival probability at different time points as well as event time, quantile, and hazard. We will bind these last three together in one tibble.
